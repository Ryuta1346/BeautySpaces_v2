...
RSpec.describe Reservation, type: :model do
  ...
  let(:salons_reservation1) { create(:salons_reservation, customer: salon1, reservation_time: "#{Date.today} 12:00:00") }
  let(:salons_reservation2) { create(:salons_reservation, customer: salon1, reservation_time: "#{Date.today + 1.days} 12:00:00") }
  let(:stylists_reservation1) { create(:stylists_reservation, customer: stylist1, reservation_time: "#{Date.today} 12:00:00") }
  let(:stylists_reservation2) { create(:stylists_reservation, customer: stylist1, reservation_time: "#{Date.today + 1.days} 12:00:00") }
  ...
  it "returns lists that display today's reservation information" do
    expect(reservation_info(salon1).only_today.size).to eq 1
  ...
  it "returns reservation's schedule" do
  end
